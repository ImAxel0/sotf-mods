...
public class AXSUI
{
    ...
    public static SContainerOptions AxMenuButton(string label, Action onClick, Color? color = null, EBackground style = EBackground.None, float height = 50f)
    {
        ...
        return SContainer.Background(Color.black.WithAlpha(0.9f), style).PHeight(height)
            - SBgButton
            .Text(label)
            .Dock(EDockType.Fill)
            .Color(color.Value)
    ...
    /// <param name="enableInput"></param>
    /// <returns></returns>
    public static SPanelOptions AxCreateMenuPanel(string id, bool closeButton, bool canMoveAndResize, string title, Vector2 size, AnchorType anchorType = AnchorType.MiddleCenter, Color? color = null, EBackground style = EBackground.RoundedStandard, bool enableInput = true)
    {
        ...
        var titleContainer = SContainer.Background((Color)color, style).PaddingVertical(10).PHeight(size.y / 5)
            - AxTextAutoSize(title).Id("menutitle");

        if (closeButton)
        {
            titleContainer.Add(AxButton("X", CloseMenuPanel, new Vector2(size.x / 20, size.y / 20), AnchorType.TopRight).Background(style).Id($"close{id}"));
        }
